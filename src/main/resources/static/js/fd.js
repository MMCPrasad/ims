...
function fdInitiate(target, pageName, tableData, formData) {
...
function setTableStatus(td) {
    if ($(td).html().trim().startsWith('active')) {
        ...
        $(td).html('<small class="badge fs--1 w-100 badge-soft-danger">Rejected</small>');
    }else if ($(td).html() === 'approved') {
        $(td).html('<small class="badge fs--1 w-100 badge-soft-success">Approved</small>');
    }
}
