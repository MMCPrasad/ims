...
function grnCreation(target, pageName, tableData, formData) {
        ...
        this.dataTable = $(target).find('#mainTable').DataTable({
            ...
            "createdRow": tableData.createdRow || function (row, data) {
                ...
                if (data['status'] === 'inactive') {
                    ...
                    $(action_td).append('<a href="javascript:void(0)" class="editrec me-1" data-toggle="tooltip" data-placement="top" title="Create GRN "><i class="fas fa-plus-circle  text-info"></i></a><a href="javascript:void(0)" class="delrec" data-toggle="tooltip" data-placement="top" title="Cancel Order"><i class="fas fa-times-circle text-danger"></i></i></a>');
                }
    ...
    this.setupCommonOperations = function () {
        ...
        $(target).find('#mainTable').on('click', '.delrec', function () {
            ...
            Swal.fire({
                ...
                showLoaderOnConfirm: true,
                preConfirm: () => {
                    return fetch('grn-creation/cancel', {
                        method: "PUT",
        ...
        $(target).find('#mainTable').on('click', '.editrec', function () {
            ...
            fetch(formData.url + '/' + id)
                    ...
                    .then(resp => {
                        obj.clearForm();
                        subCategoryText = resp.data.quoatation.request.asset.text;
                        supplierCodeText = resp.data.quoatation.recSuplier.supplier.id;
                        locationText = resp.data.quoatation.request.department.text;
                        assignedToText = resp.data.quoatation.request.requestBy.text;
                        purchasePriceText = resp.data.quoatation.recSuplier.unitPrice;
                        purchaseDateText = resp.data.issueDate;

                        // Populate labels with the extracted data
                        document.getElementById("subCategory").textContent = subCategoryText;
                        document.getElementById("supplierCode").textContent = supplierCodeText;
                        document.getElementById("location").textContent = locationText;
                        document.getElementById("assignedTo").textContent = assignedToText;
                        document.getElementById("purchasePrice").textContent = purchasePriceText;
                        document.getElementById("PurchaseDate").textContent = purchaseDateText;
        ...
        function handleActionBtnClick() {
            console.log(approval_table_id)
            let data = {
                idOfPo: approval_table_id,
                serialNumber: $('#serialNumber').val(),
                warranty: $('#warranty').val(),
                assestCondition: $('#assestCondition').val(),
            ...
            Swal.fire({
                ...
                allowOutsideClick: () => !Swal.isLoading(),
                preConfirm: () => {
                    return fetch('grn-creation/create', {
                        method: "POST",
                        body: JSON.stringify(data),
                        headers: headers
                    })
    ...
    this.initialize = function () {
        let obj = this;
        $(target).load(window.location.origin + contextPath + 'templates/purchase.html', null, function (data, status, jqXGR) {
