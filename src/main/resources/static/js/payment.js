...
function payment(target, pageName, tableData, formData) {
    ...
    this.setupCommonOperations = function () {
        ...
        $(target).find('#submitBtn').click(function () {
    ...
    this.initialize = function () {
        ...
        $(target).load(window.location.origin + contextPath + 'templates/payment.html', null, function (data, status, jqXGR) {
...
function handleActionBtnClick() {
    ...
    let paymentRows = $('.paymentFields table tbody tr').length;

    for (let i = 0; i < paymentRows; i++) {
        let paymentFields = {};
        let row = $('.paymentFields table tbody tr').eq(i);

        paymentFields.description = row.find('.description').val();
        paymentFields.quantity = row.find('.quantity').val();
        paymentFields.unitPrice = row.find('.unit_price').val();
        paymentFields.lineTotal = row.find('.line_total').val();


        paymentDetailsArray.push(paymentFields);
    }
    
//    
//    // Check if this row's radio button is selected
//                    if (page.find('input[name="budgeted"]').is(':checked')) {
//                        budgeted = i; // Row number (0-based index)
//                    }
    ...
    let data = {
        ...
        regPayee: $('#regPayee').val(),
        unRegPayeeName: $('#unregPayeeName').val() || null,
        unRegPayeeAcc: $('#unregPayeeAcc').val() || null,
        unRegPayeeBank: $('#unregPayeeBank').val(),
        invoiceNo: $('#invoiceNo').val(),
        budget: $('#budget').val(),
        pdu: $('#pdu').val(),
        recTo: $('#recommendTo').val(),
        recComment: $('#recommendComment').val(),
