...
function po(target, pageName, tableData, formData) {
        ...
        this.dataTable = $(target).find('#mainTable').DataTable({
            ...
            "createdRow": tableData.createdRow || function (row, data) {
                ...
                if (data['status'] === 'inactive') {
                    ...
                    $(action_td).append('<a href="javascript:void(0)" class="editrec me-1" data-toggle="tooltip" data-placement="top" title="Create PO"><i class="fas fa-plus-circle h6 text-info" style="color: #000000;"></i></a>');
                }
    ...
    this.setupCommonOperations = function () {
        ...
        $(target).find('#mainTable').on('click', '.editrec', function () {
            ...
            fetch(formData.url + '/' + id)
                    ...
                    .then(resp => {
                        ...
                        document.getElementById("requestId").textContent  = resp.data.request.requestId;
                        document.getElementById("requestBy").textContent  = resp.data.request.requestBy.text;
                         document.getElementById("department").textContent  = resp.data.request.department.text;
                          document.getElementById("asset").textContent  = resp.data.request.asset.text;
                          document.getElementById("quantity").textContent  = resp.data.quantity;
                          document.getElementById("budgetAvailability").textContent  = resp.data.request.budgetAvailability;
                          document.getElementById("budgetLine").textContent  = resp.data.request.budgetLine.text;
                          document.getElementById("justification").textContent  = resp.data.request.justification;
                           document.getElementById("supplier").textContent  = resp.data.recSuplier.supplier.text;
                           document.getElementById("itemName").textContent  = resp.data.recSuplier.itemName;
                           document.getElementById("description").textContent  = resp.data.recSuplier.description;
                           document.getElementById("total").textContent  = resp.data.recSuplier.total;
        ...
        $(target).find('#createBtn').click(function () {
...
function handleActionBtnClick() {
    ...
    Swal.fire({
        title: "Are you sure?",
        text: "This purchase order will be created!",
        icon: "question",
        showCancelButton: true,
        confirmButtonColor: "#4bbf73",
        cancelButtonColor: "#495057",
        ...
        allowOutsideClick: () => !Swal.isLoading(),
        preConfirm: () => {
            return fetch("po-creation/create", {
                method: "POST",
