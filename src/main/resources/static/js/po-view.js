function poView(target, pageName, tableData, formData) {
    ...
    this.setupCommonOperations = function () {
        ...
        $(target).find('#mainTable').on('click', '.editrec', function () {
            ...
            fetch(formData.url + '/' + id)
                    ...
                    .then(resp => {
                        obj.clearForm();
                        document.getElementById('poNo').textContent = resp.data.poNo;
                        document.getElementById('issueDate').textContent = resp.data.issueDate;
                        document.getElementById('preferredName').textContent = resp.data.quoatation.recSuplier.supplier.preferredName;
                        document.getElementById('permanentAddressL1').textContent = resp.data.quoatation.recSuplier.supplier.permanentAddressL1;
                        document.getElementById('permanentAddressL2').textContent = resp.data.quoatation.recSuplier.supplier.permanentAddressL2;
                        document.getElementById('permanentAddressL3').textContent = resp.data.quoatation.recSuplier.supplier.permanentAddressL3;
                        document.getElementById('residentialCity').textContent = resp.data.quoatation.recSuplier.supplier.residentialCity.text;
                        document.getElementById('telephones').textContent = resp.data.quoatation.recSuplier.supplier.telephones[0].number;
                        document.getElementById('itemName').textContent = resp.data.quoatation.recSuplier.itemName;
                        document.getElementById('description').textContent = resp.data.quoatation.recSuplier.description;
                        document.getElementById('quantity').textContent = resp.data.quoatation.quantity;
                        document.getElementById('unitPrice').textContent = resp.data.quoatation.recSuplier.unitPrice;
                        document.getElementById("expiryDate").innerText = resp.data.expiryDate;
                   document.getElementById('comment').textContent = resp.data.comment;
                        document.getElementById('subTotal').textContent = resp.data.quoatation.recSuplier.total;
                        document.getElementById('otherCharges').textContent = resp.data.quoatation.recSuplier.tax;
                            document.getElementById('fulltotal').textContent = resp.data.quoatation.recSuplier.total;


                        const quantity = parseFloat(resp.data.quoatation.quantity);
                        const unitPrice = parseFloat(resp.data.quoatation.recSuplier.unitPrice);
        ...
        $(target).find('#pdfBtn, #printBtn').click(function () {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "This Purchase Order will be Downloaded",
                icon: "question",
                showCancelButton: true,
                confirmButtonColor: "#4bbf73",
                cancelButtonColor: "#495057",
...
//                    window.print();
        ...
        function PrintElem() {
            ...
            a.document.write('<style>.align-right { text-align: right; } .po-table thead th { background-color: #2c7be5; color: white; } .table-borderless tbody tr td, .table-borderless tbody tr th, .table-borderless thead tr th, .table-borderless thead tr td { border: none !important; } .text-end { text-align: right; } h1 { color: #2c7be5; }</style>');
    ...
    this.initialize = function () {
        ...
        $(target).load(window.location.origin + contextPath + 'templates/po.html', null, function (data, status, jqXGR) {
