function productIntiate(target, pageName, endPoint, tableData, formData) {
  ...
  $(target).find('#mainTable').on('click', '.editrec', function () {
    ...
    this.initialize = function () {
        ...
        $(target).load(
                window.location.origin + contextPath + "templates/approval.html",
                null,
...
function setTableStatus(td) {
    if ($(td).html().trim().startsWith("approved")) {
        $(td).html(
                '<small class="badge fs--1 w-100 badge-soft-success">Approved</small>'
                );
    } else if ($(td).html() === "rejected") {
        $(td).html(
                '<small class="badge fs--1 w-100 badge-soft-danger">Rejected</small>'
                );
...
function findNestedProperty(obj, property) {
    let nestedProps = property.split('.');
    let currentObj = obj;
    for (let prop of nestedProps) {
        if (currentObj[prop] !== undefined && currentObj[prop] !== null) {
