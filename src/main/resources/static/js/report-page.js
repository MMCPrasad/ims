function initiateReportPage(target, pageName, tableData, formData) {
    ...
    this.setupCommonOperations = function () {
        ...
        $(target).find('#mainTable').on('click', '.editrec', function () {
            ...
            fetch(formData.url + '/' + id)
                    ...
                    .then(resp => {
                        obj.clearForm();
                        $(target).find('#formSection').find('label').each(function () {
                            let labelFor = $(this).attr('for');
                            let data = resp.data[labelFor];
                            if (data !== null && typeof data === 'object' && data.hasOwnProperty('text')) {
                                $("#" + labelFor + "").text(data.text); // Use nested object's text attribute
                            } else {
                                if (data !== undefined && data !== null) {
                                    $("#" + labelFor + "").text(data); // Use attribute value directly
                                }
                            }
                        });
    ...
    this.initialize = function () {
        ...
        $(target).load(window.location.origin + contextPath + 'templates/report.html', null, function (data, status, jqXGR) {
